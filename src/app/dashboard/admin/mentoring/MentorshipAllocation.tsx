...
import { Network, Users, UserCheck, Search, Filter, ArrowRightLeft, Share2, Download } from 'lucide-react';
...
export default function MentorshipAllocation() {
    ...
    // Manual Allocation State (tracks only *unsaved changes* overriding DB)
    const [manualAllocations, setManualAllocations] = useState<Record<string, string | null>>({}); // { menteeId: mentorId }
    ...
    const filteredMentees = mentees.filter(m => m.name.toLowerCase().includes(menteeSearchQuery.toLowerCase()) || m.email.toLowerCase().includes(menteeSearchQuery.toLowerCase()));
    ...
    const handleGenerateAllocation = () => {
        ...
        let studentsToDistribute = [...mentees];
        ...
        const previewMap = [];
        // Only distribute students that don't have a mentor globally OR are forced?
        // Let's keep it simple and just do what we did before.
        for (let i = 0; i < studentsToDistribute.length; i++) {
            const student = studentsToDistribute[i];
            const mentor = mentors[i % mentors.length]; // Round robin assignment
    ...
    const handleCommitAutoAllocation = () => {
        ...
        // Save mapping to database (zustand store)
        generatedPreview.forEach(mapping => {
            updateUser(mapping.student.id, { mentorId: mapping.mentor.id });
        });
    ...
    const handleSaveManualAllocation = () => {
        ...
        setManualAllocations({}); // Clear staged changes as they are now in DB
    ...
    const generatePDF = async (): Promise<jsPDF | null> => {
        try {
            ...
            // 1. Institute Logo (Mocked as text)
            pdf.setFontSize(12);
            ...
            pdf.setFontSize(22);
            pdf.text('Institute Name', pageWidth / 2, y, { align: 'center' });
            y += 10;

            // 3. Mentorship Program
            pdf.setFontSize(16);
            pdf.text('Mentorship Program', pageWidth / 2, y, { align: 'center' });
            y += 8;

            // 4. Mentor- Mentee Allocation List
            pdf.setFontSize(14);
            pdf.setFont("helvetica", "normal");
            pdf.text('Mentor- Mentee Allocation List', pageWidth / 2, y, { align: 'center' });
            y += 15;
            ...
            if (mentorGroups.length === 0) {
                pdf.setFontSize(12);
                pdf.text("No mappings available to export.", 14, y);
            }

            for (const group of mentorGroups) {
                checkPageBreak(20);
                
                // Mentor Name, Designation and Department
                pdf.setFont("helvetica", "bold");
                pdf.setFontSize(12);
                pdf.text(`${group.mentor.name}, Faculty, Department of Medicine`, 14, y);
                y += 6;
                ...
                for (let i = 0; i < group.students.length; i++) {
                    ...
                    // Using ID / Email prefix as a mock registration number since it's not in the global user schema natively yet
                    const mockRegNo = s.email.split('@')[0].toUpperCase();
                    pdf.text(`  •   ${s.name}, Reg No: ${mockRegNo}`, 14, y);
                    y += 6;
                }
                
                pdf.setTextColor(0, 0, 0); // reset color
                y += 6; // Extra space before next mentor
            }
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'Mentorship Allocations',
                    text: 'Here are the latest mentorship allocations from MedEduAI.'
                });
            } else {
                // Fallback to download if sharing not supported
                pdf.save('Mentorship_Allocations.pdf');
            }
        } catch (error) {
            console.error('Error sharing PDF:', error);
            // Don't alert if the user just cancelled the share dialog
            if ((error as Error).name !== 'AbortError') {
                alert('Sharing failed or was cancelled. Try downloading instead.');
            }
        }
    };
        ...
        <div className="w-full text-left">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Map students to faculty mentors either manually or through bulk auto-distribution.</p>
                </div>
                ...
                <div className="bg-amber-50/50 border border-amber-200 rounded-3xl p-8 mb-8">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600">Select batch parameters to instantly map available mentees.</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                        <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                            ...
                            >
                                <option value="1">2021-2022 MBBS</option>
                                <option value="2">2022-2023 MBBS</option>
                                <option value="3">2023-2024 MBBS</option>
                                <option value="4">2024-2025 MBBS</option>
                                <option value="5">2025-2026 MBBS</option>
                            </select>
                        </div>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {generatedPreview.map((mapping, idx) => (
                                            <tr key={idx} className="hover:bg-slate-50/50">
                                                <td className="px-4 py-3 font-medium text-slate-900">{mapping.student.name}</td>
                    ...
                    {/* Display currently saved allocations */}
                    <div className="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <div id="allocation-table-container" className="overflow-x-auto max-h-[400px] overflow-y-auto bg-white p-2">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {mentees.filter(m => m.mentorId).length === 0 ? (
                                        <tr>
                                            <td colSpan={2} className="px-4 py-8 text-center text-slate-500 italic">No students are currently allocated.</td>
                                        </tr>
                                    ) : (
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-slate-50/50">
                                                    <td className="px-4 py-3 font-medium text-slate-900">
                                                        {mentee.name} <span className="text-xs text-slate-400 font-normal ml-1">({mentee.email})</span>
                                                    </td>
                                                    <td className="px-4 py-3 text-emerald-700 font-medium flex items-center gap-2">
                                                        <UserCheck className="w-4 h-4" />
                                                        {assignedMentor ? assignedMentor.name : 'Unknown Mentor'}
                                                    </td>
                                                </tr>
                                            );
                                        })
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-[500px]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {filteredMentors.length > 0 ? filteredMentors.map((mentor, i) => {
                                ...
                                // Calculate how many mentees will be mapped to this mentor including staged changes
                    ...
                    <div className="bg-white border text-left border-slate-200 rounded-2xl shadow-sm flex flex-col h-[500px]">
                        <div className="p-4 border-b border-slate-100 bg-slate-50">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button className="p-2 bg-slate-100 text-slate-600 rounded-lg border border-slate-200 hover:bg-slate-200 transition-colors">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {filteredMentees.length > 0 ? filteredMentees.map((mentee) => {
                                ...
                                // Calculate "effective" assignment (combining DB state with unsaved manual changes)
                                ...
                                // Did we make a change to this user that isn't saved yet?
                                const hasUnsavedChange = manualAllocations[mentee.id] !== undefined;
                                
                                // See if they were saved in the DB originally (and not being removed currently)
                                const isSavedInDb = mentee.mentorId && !hasUnsavedChange;
                                ...
                                <div key={mentee.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 rounded-xl border border-slate-100 bg-slate-50/50">
                                    <div className="flex flex-col mb-2 sm:mb-0">
                                        ...
                                        {isAssignedToOther && (
                                            <p className="text-xs text-amber-500 font-medium mt-1">Assigned to another mentor</p>
                                        )}
                                        {isSavedInDb && !isAssignedToOther && !isAssignedToCurrent && (
                                            <p className="text-xs text-emerald-600 font-medium mt-1">Already allocated</p>
                                        )}
                                    </div>
                                    <button 
                                        onClick={() => {
                                            ...
                                            setManualAllocations(prev => {
                                                if (isAssignedToCurrent) {
                                                    // Unassign if clicked again (mark null locally to clear from DB on save)
                                                    return { ...prev, [mentee.id]: null };
                                                } else {
                                                    // Assign to current locally
                                                    return { ...prev, [mentee.id]: activeMentorId };
                                                }
                                            });
                                        }}
