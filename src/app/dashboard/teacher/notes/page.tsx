...
export default function TeacherLMSNotes() {
    ...
    const currentCourse = coursesList[0]; // Simplified for dashboard
        ...
        <div className="fixed inset-0 z-[100] bg-slate-50 flex overflow-hidden font-sans">
            ...
            <aside className={`fixed inset-y-0 left-0 w-[300px] flex-shrink-0 bg-white border-r border-slate-200 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-[110] transform transition-transform duration-300 md:relative md:translate-x-0 ${showSidebar ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <div className="p-6 pb-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Course Curriculum</p>
                    <select
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {availableSections.map(section => {
                        const isExpanded = expandedSections[section.id] !== false; // default true
                        const publishedTopics = section.topics.filter(t => t.generatedNotes);
                                ...
                                <AnimatePresence>
                                    {isExpanded && (
                                        <motion.div
                                            ...
                                            className="ml-2 pl-2 border-l-2 border-slate-100 mt-1 space-y-1 overflow-hidden"
                                        >
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        ...
                                        {activeTab === 'ppt' && contentMap.ppt && (
                                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-8 border border-amber-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                                                <div className="flex items-center gap-3 mb-6">
                                                    <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center text-amber-600">
                                                        <Presentation className="w-5 h-5" />
                                                    </div>
                                                    <h3 className="text-xl font-bold text-amber-900">PPT Script / Layout</h3>
                                                </div>
                                                <div className="bg-white/60 p-6 rounded-2xl border border-white font-medium text-slate-700 leading-relaxed shadow-inner prose max-w-none">
                                                    <ReactMarkdown remarkPlugins={[remarkGfm]}>{contentMap.ppt}</ReactMarkdown>
                                                </div>
                                            </div>
                                        )}
